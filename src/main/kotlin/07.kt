...
fun main() {
    ...
    val dir = root
        .directories()
        .findDirectoryClosestTo( root.size() - 40000000)

    println(dir)
}

private fun Directory.directories(): List<Directory> {
    return listOf(this) + this.subDirectories.flatMap { it.directories() }
}

private fun Directory.size(): Int {
    return this.files.sumOf { it.size } + this.subDirectories.sumOf { it.size() }
}

fun String.readInput() = this.split("\n").filterNot { it == "$ ls" }

fun List<String>.createDirectories(): Directory {
    ...
    this.drop(1).forEach { line ->
        if (line.startsWith("dir")) {
            currentDirectory.subDirectories.add(
                Directory(line.split(" ")[1], currentDirectory)
            )
        } else if (line == "$ cd ..") {
          currentDirectory = currentDirectory.parent!!
        } else if (line.startsWith("$ cd")) {
            val newDirName = line.split(" ")[2]
            currentDirectory = currentDirectory.subDirectories.first { it.name == newDirName }
        } else {
            currentDirectory.files.add(
                line.split(" ").let { File(it[1],it[0].toInt()) }
            )
        }
    }
